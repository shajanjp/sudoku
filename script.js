...
function isSolved () {
  ...
  for (var cell = 1; cell < 82; cell++) {
    if (getCellValue(cell) == '') { solved = false }
  }
  return solved
}

function getCellValue (cell) {
  if (document.getElementById(cell).value) { return parseInt((document.getElementById(cell).value || false)) } else { return '' }
}
...
function setAllValues (dataset) {
  for (var cell = 1; cell < 82; cell++) {
    if (dataset[cell]) { setCellValue(cell, dataset[cell]) }
...
function fillData () {
  ...
  for (var cell = 1; cell < 82; cell++) {
    ...
    } else if (dataset[cell - 1] == 0) { setCellValue(cell, '') }
  }
}
...
function getPossibilities (cell) {
  ...
  if (poss.length == 1) { setCellValue(cell, poss[0]) }
...
function updatePossibilites () {
  for (var cell = 1; cell < 82; cell++) {
    if (getCellValue(cell) == '') { getPossibilities(cell) }
  }
}
